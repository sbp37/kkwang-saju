...
// 궁합도 같은 원칙으로, 몇 점이 아니라 "어떤 결의 관계인지"를 설명한다.
...
function analyzeMatch(a, b) {
  ...
  let 결, 결설명;
  if (stemHap && (branchRel === '육합' || branchRel === '삼합')) {
    결 = '서로 끌어당기는 결';
    결설명 = '중심 기운끼리도, 가까운 자리끼리도 서로를 붙잡는 구조야. 처음부터 편하게 느껴지는 조합이야.';
  } else if (stemHap) {
    결 = '마음이 먼저 붙는 결';
    결설명 = '두 사람의 중심 기운이 서로를 끌어당겨. 이유를 설명하기 전에 마음이 먼저 가는 쪽이야.';
  } else if (branchRel === '육합' || branchRel === '삼합') {
    결 = '같이 있으면 편한 결';
    결설명 = '가장 가까운 자리끼리 잘 묶여 있어. 크게 애쓰지 않아도 생활이 맞춰지는 조합이야.';
  } else if (branchRel === '충') {
    결 = '부딪히며 커지는 결';
    결설명 = '가까운 자리끼리 정면으로 만나. 지루할 틈은 없는데, 같은 문제로 반복해서 부딪힐 수도 있어.';
  } else if (보완 === '서로') {
    결 = '서로 채워주는 결';
...
{ id:'같이-일지합', slot:'같이있을때', weight:96,
  when:m => m.branchRel === '육합' || m.branchRel === '삼합',
  text:m => `두 사람의 가장 가까운 자리가 서로 묶여 있어. 생활 리듬이나 편안한 거리감이 자연스럽게 맞춰지는 편이라, 같이 지내는 데 드는 힘이 적어.` },
{ id:'같이-일지같음', slot:'같이있을때', weight:90,
  when:m => m.sameBranch,
  text:m => `두 사람이 가까운 자리에 같은 글자를 갖고 있어. 편한 게 비슷하고 불편한 것도 비슷해서, 설명 없이 통하는 게 많아.` },
...
{ id:'갈등-일지충', slot:'부딪힘', weight:96,
  ...
  text:m => `가장 가까운 자리끼리 정면으로 만나는 구조야. 생활 방식이나 편한 거리감이 서로 달라서, 사소한 데서 반복해서 부딪히기 쉬워. 대신 서로를 흔들어 깨우는 면도 있어서 지루해지지는 않아.` },
...
{ id:'오래-십신', slot:'오래가려면', weight:84,
  ...
  text:m => {
    const T = {
      ...
      편재:'같이 벌인 일이 흐지부지되지 않게, 끝을 확인하는 사람을 정해둬.',
